package fpinscala.chapters.chapter10

import fpinscala.chapters.chapter7.Nonblocking.Par

/**
  * Created by seung on 2017/09/03.
  */
trait Monoid[A] {
  def op(a1: A, a2: A): A
...
object Monoid {
  ...
  /**
    ...
    * import Prop._
    * *
    * def monoidLaws[A](m: Monoid[A], gen: Gen[A]): Prop = //Associativity
    * forAll(for {
    * x <- gen
    * y <- gen
    * z <- gen
    * } yield (x, y, z))(p =>
    *m.op(p._1, m.op(p._2, p._3)) == m.op(m.op(p._1, p._2), p._3)) && // Identity
    * forAll (gen) ((a: A) =>
  ...
  import fpinscala.chapters.chapter7.Nonblocking._
  // infix syntax for `Par.map`, `Par.flatMap`, etc
  import fpinscala.chapters.chapter7.Nonblocking.Par.toParOps
